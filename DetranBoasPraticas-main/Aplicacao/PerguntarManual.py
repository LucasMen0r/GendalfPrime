...
def LimparRespostaDeepSeek(textobruto: str) -> str:
    ...
    # Remove the <think>...</think> reasoning block generated by DeepSeek R1
    texto_limpo = re.sub(r"<think>.*?</think>", "", textobruto, flags=re.DOTALL)
    # Remove tool calls if any
    texto_limpo = re.sub(r"<tool_call>.*?</tool_call>", "", texto_limpo, flags=re.DOTALL)
        ...
        resposta = requests.post(
            ...
            json={
                ...
                "options": {
                    "temperature": 0,
                    "num_ctx": 4096,
                    "num_predict": 1400 if modo_professor else 1000,
                },
            },
            stream=True,
            timeout=(10, 900),
